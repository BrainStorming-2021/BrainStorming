...
# 65~90 # 대문자
# 97~122# 소문자
def solution(input_arr, push):
    for x in range(len(input_arr)):
        if input_arr[x].isupper():
            input_arr[x] = chr((ord(input_arr[x])-ord('A')+push)%26+ord('A'))
        elif input_arr[x].islower():
            input_arr[x] = chr((ord(input_arr[x])-ord('a')+push)%26+ord('a'))
        else :
            # else 처리해주시
            print(0)
