...
예를 들어, arr = [3,2,1,4]이고 k = 3을 선택하는 팬케이크 플립을 수행했다면, 
하위 어레이 [3,2,1]을 반전시켰으므로, k = 3에서 팬케이크 플립 후 arr = [1,2,3,4]입니다.

정렬된 팬케이크 플립 시퀀스에 해당하는 k-값 배열을 반환합니다. 
배열을 10 * arr.length 플립 내에서 정렬하는 유효한 답변은 모두 올바른 것으로 판단됩니다.
